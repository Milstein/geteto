...
import br.jabuti.probe.desktop.DefaultProber;
import br.jabuti.probe.desktop.DefaultTraceReader;
...
public class HostTraceReader extends DefaultTraceReader
{
	...
	protected int readPaths()
	{
		...
		do {
			try {
				...
				} else if (tipo.equals(this.getClass().toString())) {
					v = readOnePath();
				}
				if (v != null)
					paths.put(v[0], v[1]);
	...
	protected Object[] readOnePath() throws Exception
	{
		...
		ProbedNode pdn = new HostProbedNode(host, thrd, obj, claz, Integer.parseInt(meth), "");
	...
	*/
	...
	/**
	 * This method takes a ProbedNode and a path ina String[] form and returns the string
	 * correspondent to that path
	 */
	...
	/**
	 * This method takes an string and returns a Vector where the firsts element is a ProbedNode
	 * object and the second a String[] object with a path for that ProbedNode
	 */
	static public List xloadFromString(String x)
	{
		...
		ProbedNode pdn = new HostProbedNode(host, thread, obj, clazz, Integer.parseInt(metodo), "");
