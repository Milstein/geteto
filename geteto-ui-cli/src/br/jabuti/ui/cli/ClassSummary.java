...
public class ClassSummary
{
	...
	public static void usage()
	{
		...
		System.out.println("\nUSAGE:");
		System.out
						.println("java graph.ClassSummary [options] -i <class files> | -jar <compressed file>\n");
		System.out
						.println("      -i <class files>        A list of classes to be instrumented these classes");
		System.out
						.println("                              should be reachable from the <main_class>.\n");
		System.out.println("      -jar <compressed file>  A compressed .jar or .zip file.\n");
		System.out
						.println("      [options] could be: [-h | -v | -du | -li | -do | -hi | -cfg | -all ]");
		...
		System.out
						.println("      -li:                    Shows Alive Variables of each Node, if any.");
		System.out
						.println("      -do:                    Shows Dominators and Inverse Dominators of each Node, if any.");
		System.out
						.println("      -ch:                    Shows Primary and Secondary Children of each Node, if any.");
		System.out
	...
	/**
	 * Capturar e imprimir as informa��es sobre defini��es e usuos de vari�veis. Gerar o CFG por
		...
		for (int i = 0; i < methods.length; i++) {
			try {
				...
				for (int j = 0; j < g.size(); j++) {
					...
					if (detailed) {
						System.out.println("\t\t\t\t" + pred.getLabel() + " PC: " + pred.getStart()
										+ " to " + pred.getEnd());
				...
				if (detailed) {
					...
					while (!(decisionBlocks.isEmpty())) {
						...
						System.out.println("\t\t\t\t" + pred.getLabel() + " Start: "
										+ pred.getStart() + " End: " + pred.getEnd());
		...
		try {
			...
			if (gfc != null) {
				...
				for (int i = 0; i < fdt.length; i++) {
					...
					} else if ((current.getPrimNext().size() == 0)
		...
		try {
			...
			if (bbDom != null) {
				for (int z1 = 0; z1 < bbDom.size(); z1++) {
					...
					dotFile.println("\t" + dtn.getLabel() + " [label=\""
									+ dtn.getOriginalNode().getLabel() + "\"];");
