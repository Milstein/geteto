...
public class Dao {
	...
	public Dao(Properties prop) {
		conn = br.icmc.usp.jabuti.service.persistence.Connection.getConnection(prop);
	}
	...
	public JabutiServiceProject[] getProjects() {
		ArrayList<JabutiServiceProject> projs =
				new ArrayList<JabutiServiceProject>();
		
		try {
			JabutiServiceProject jsp;
			Statement st = conn.createStatement();
			ResultSet rs = st.executeQuery("SELECT id FROM project");
			while (rs.next()) {
				jsp = get(rs.getString("id"));
				projs.add(jsp);
			}
			rs.close();
			st.close();
		...
		return projs.toArray(new JabutiServiceProject[0]);
	}
