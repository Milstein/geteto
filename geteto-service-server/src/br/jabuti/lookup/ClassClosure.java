...
/**
 * This class is used to explore a program structure.
 * Given a class path and a base class X, this class has a few methods to find out which
 * other classes X depends on.
 * The programer can specify whether system classes should be included or excluded
 * in this search and can also determine which packages should be ignored in the
 * search.
 * @author delamaro
 *
 * 
...
public class ClassClosure {
    static final private String[] sysPrefix =
            new String[] { "java.", "javax.lang", "org.omg" };
     ...
     */
    public String[] getClosure(String className, boolean noSys,
            String toAvoid) {
        String s = className;
    ...
    public String[] accessedClasses(JavaClass javaClass) {
        ...
        for (int i = 0; i < ct.length; i++) {
            if (ct[i] instanceof ConstantClass) {
                ...
                ConstantUtf8 cutf =
                        (ConstantUtf8) cp.getConstant(cc.getNameIndex());
